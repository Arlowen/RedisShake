...
**只做增量**：`sync_rdb = false` 的含义是不把快照内容写到目标端，不代表源端就一定不会发送快照。复制协议本身仍可能要求先传输 RDB，再进入增量阶段。

**云服务兼容性**：是否支持 PSync，取决于具体产品版本、实例形态和权限配置。对于云厂商实例，建议先结合 [迁移模式选择](../guide/mode.md) 与服务商说明确认。
