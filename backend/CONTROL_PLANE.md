...
Connection CRUD, Redis prechecks, worker process management, and the Web UI are the next implementation phases.
...
The server can start without a master key while no encrypted credentials exist. A later phase will reject creating a password-bearing connection until `REDISSHAKE_MASTER_KEY` is configured.
