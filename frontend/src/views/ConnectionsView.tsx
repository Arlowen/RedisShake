...
export default function ConnectionsView() {
  ...
  return <div className="page-wrap">
    <PageHeader title="连接管理" description="保存并测试 Redis 连接，凭据由控制面加密。">
      {store.items.length ? <Button type="primary" onClick={() => navigate('/connections/new')}>新建连接</Button> : null}
    </PageHeader>
    ...
    {store.loading && !store.loaded ? <div className="skeleton-list">{[0, 1, 2, 3].map((item) => <div key={item} className="skeleton-row" />)}</div>
      : !store.items.length ? <EmptyState title="添加 Redis 连接" description="保存源端或目标端连接，创建任务时直接选择。"><Button type="primary" onClick={() => navigate('/connections/new')}>新建连接</Button></EmptyState>
